...
log_path="logs/par_relaxed_smoke_${timestamp}.log"
...
for c in 2 4 8; do
  ./build/benchmarks/bench_smoke --engine par_relaxed --seed 1 --vertices 32 --updates 100 --delta-cap 8 --batch-size 1 --palette-multiplier "${c}" --max-rounds 4
  ./build/benchmarks/bench_smoke --engine par_relaxed --seed 1 --vertices 32 --updates 100 --delta-cap 8 --batch-size 4 --palette-multiplier "${c}" --max-rounds 4
done
